# Program in Python to create a Snake Game with enhancements (animation, fonts, pause, restart)
...
def next_turn(snake, food):
    ...
    if check_collisions(snake):
        game_over_animation()  # Game over animation instead of sound
    else:
...
# Function to pause and resume game 
def toggle_pause():
    global paused
    paused = not paused
    pause_button.config(text="Resume" if paused else "Pause")

    if not paused:  # Resume the game
...
def game_over_animation():
    ...
    for i in range(5):
        canvas.create_text(
            ...
            font=('consolas', 70),
            text="GAME OVER", fill="red" if i % 2 == 0 else "yellow",
            tag="gameover"
        )
        window.update()
        window.after(200)
...
# Pause Button
pause_button = Button(window, text="Pause", command=toggle_pause)
pause_button.pack()
...
window.bind('r', lambda event: restart_game())  # 'r' to restart the game
